...
describe 'Pruebas para crear un robot en el tablero' do
    ...
    it "Deberia haber un robot en la posicion final '2-2-derecha'" do
        @board.createNewRobot(2,2,'derecha')
        expect(@board.getRobotFinalPosition(1)).to eq '2-2-derecha'
    end

    it "No deberia haber un segundo robot en la posicion final '2-2-derecha'" do
        @board.createNewRobot(2,2,'derecha')
        expect(@board.getRobotFinalPosition(5)).to eq 'no-robot'
    end

    it "Dado un robor en la posicon '2-2-derecha' deberia estar en la posicion '4-2-derecha' si lo movemos dos veces" do
        @board.createNewRobot(2,2,'derecha')
        @board.moveRobot(1)
        @board.moveRobot(1)
        expect(@board.getRobotFinalPosition(1)).to eq '4-2-derecha'
    end
